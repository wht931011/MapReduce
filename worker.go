...
func (wk *Worker) register(master string) {
	...
	args.Worker = wk.name
	// let worker connect to master
	// "Master.Register" is a function defined in master.go
	// "Master.Register" parses the worker info to master.registerchannel and master.worker
...
func RunWorker(MasterAddress string, me string,
	...
	wk.nRPC = nRPC
	rpcs := rpc.NewServer() // create a server for worker
	rpcs.Register(wk) // register worker
	...
	wk.l = l
	// different from rpcs.Register, this register is defined above.
	// register let worker connect to master.(put worker to the master regesterchannel)
